...
load('data/states.mat','N','n_tr','state_name'); 

% this factor is used to modify the transport matrix and accouts for the 
% travel decline during the beginning period of epidemic compared to the data set
alpha_tr = 0.5;   
n_tr = alpha_tr*n_tr;
...
load('data/Itrue.mat','I_true','T');

beta = 0;     % No quarantined compartment
gamma = 0.5;


%% Ensenble Kalman filter parameters

% Ranges of initial distributions
r_range = [0.1,0.3];
b_range = [1,1.5];
alpha_range = [0,500];
U_range = [0,200];
...
%% Initial distribution for parameters to be analyzed %%
